var colors = require('colors'); // List of test describers to be patched.
...
exports.descriptorsByEnvironmentPatcher = function descriptorsByEnvironmentPatcher() {
  ...
  function buildFunctionForEnv(_ref) {
    ...
    if (shouldReturnDescriber) {
      ...
      return function () {
        throw new Error(colors.red("Seems that you are doing a " + descriptorName + "." + env + ".only but you are running the tests for the " + (isNode ? environments.SERVER : environments.CLIENT) + "\n\n"));
      };
    } else {
      return function (title) {
        return console.warn(colors.yellow("skiping on the " + (isNode ? environments.SERVER : environments.CLIENT) + " '" + descriptorName + "('" + title + "')\n"));
      };
    }
  }
  /**
